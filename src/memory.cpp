...
namespace rv32 {
    ...
    Memory::Memory(std::size_t size) noexcept
        : data_(size, 0)
    {
    }
    ...
    std::expected<HalfWord, Trap> Memory::read16(Word addr) const noexcept
    {
        ...
        if (address >= data_.size() || data_.size() - address < 2) {
            return std::unexpected(Trap::LoadAccessFault);
        }

        const HalfWord low  = data_[address];
        const HalfWord high = data_[address + 1];
    ...
    std::expected<Word, Trap> Memory::read32(Word addr) const noexcept
    {
        ...
        if (address >= data_.size() || data_.size() - address < 4) {
            return std::unexpected(Trap::LoadAccessFault);
        }
